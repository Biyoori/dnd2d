...
class CharacterCreator:
    ...
    def _create_character(self) -> Character:
        if not all([self.character_name, self.character_class, self.character_race, self.ability_scores, self.skills]):
            raise ValueError("Character creation incomplete. Make sure you're not missing anything during creation.")
        return CharacterFactory.create_character(self.character_name, self.character_class, self.character_race, self.ability_scores, self.skills)