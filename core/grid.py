...
class Grid:
    ...
    def updatePath(self, startPos, currentPos, maxMovement):
        path = self.calculatePath(startPos, currentPos)
        cost = self.calculateMovementCost(path)
    ...
    def calculateMovementCost(self, path):
        return len(path[1:]) * 5
